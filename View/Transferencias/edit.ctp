<div class="widget-box">
  ...
  <div class="widget-body" style="overflow-y: auto; max-height: 400px; min-width: 650px;">
    <div class="widget-main">
      <?php
      echo $this->Form->create('Transferencia', array('class' => 'form-horizontal'));
      ...
      <div class="row-fluid">
          <div class="input-prepend">
            ...
            <select name="data[Pedido][setores_id]" class="input-xlarge select" onchange="getDescricao(this);">
              ...
              foreach ($setores as $key => $value) {
                $selected = $key == $this->request->data['Transferencia']['setores_id'] ? 'selected' : null;
          ...
          <div class="input-prepend">
            ...
            <select name="data[Transferencia][setores_id_entrega]" class="input-large select" onchange="getDescricao(this);">
              <option value='0'></option>
              <?php
              foreach ($setores as $key => $value) {
                $selected = $key == $this->request->data['Transferencia']['setores_id_entrega'] ? 'selected' : null;
                echo "<option value='{$key}' {$selected}>{$value}</option>";
              }
              ?>
            </select>
          ...
          <div class="input-prepend">
              ...
              <<?php echo $this->Form->input('situacao', array('label' => false, 'div' => false, 'type' => 'select', 'options' => array('1'=>'Pendente','2'=>'Atendido', '3'=>'Em Aberto'), 'required' => true, 'class' => 'input-medium')); ?>
          </div>
      </div>
      ...
      <span class="materiais" >
        <?php foreach ($itens as $id => $item) { ?>
          <div class="row-fluid" style="margin-top: 10px;">
            ...
            <div class="input-prepend">
              <label>Qtd. Transferida</label>
              <span class="add-on">
                <i class="icon-plus"></i>
              </span>
              <span class="input-icon input-icon-right">
                <input type="number" name="material[transferencia][]" class="input-mini" min="0" style="width: 40px;" value="<?php echo $item['ItemTransferencia']['quantidade_transferida']; ?>" />
              </span>
            </div>

                <input type="hidden" name="material[equipamentos_id][]" class="input-mini" min="0" style="width: 40px;" value="<?php echo $item['ItemTransferencia']['equipamentos_id']; ?>" />

            &nbsp;&nbsp;
            <div class="input-prepend">
                <label>Criar CI</label>
                <span class="add-on">
                  <i class="icon-exchange"></i>
                </span>
                <span class="input-icon">
                  <select name="material[realizar_ci][]" class="input-small select" onchange="getDescricao(this);">
                    <option value='0'></option>
                    <?php
                    foreach ($tipos as $key => $value) {
                      $selected = $key == $item['ItemTransferencia']['realizar_ci'] ? 'selected' : null;
                      echo "<option value='{$key}' {$selected}>{$value}</option>";
                    }
                    ?>
                  </select>
            ...
            <div class="input-prepend">
              ...
              <span class="input-icon input-icon-right">
                ...
                <input type="hidden" name="material[item][]" value="<?php echo $item['ItemTransferencia']['id']; ?>" />
                <input type="hidden" name="material[pedidos_id][]" value="<?php echo $item['ItemTransferencia']['pedidos_id']; ?>" />
                <input type="hidden" name="material[orcamentos_id][]" value="<?php echo $item['ItemTransferencia']['orcamentos_id']; ?>" />
              </span>
            </div>
        ...
        <div class="row-fluid" style="margin-top: 10px;">
          ...
          <div class="input-prepend">
            ...
            <span class="input-icon input-icon-right">
              <input type="number" name="material[transferencia][]" class="input-mini quantidade" min="0" style="width: 40px;" />
            </span>
          </div>
