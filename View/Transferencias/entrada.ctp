              ...
              <select name="select1" id="select1" multiple="multiple" style="width: 100%; height: 120px;">
                <?php if(sizeof($itens)){ ?>
                  <?php foreach ($itens as $value) {  ?>
                    <?php if($value['ItemTransferencia']['realizou_entrada'] <> 1) { ?>
                    <option
                      value="<?php echo $value['ItemTransferencia']['pedidos_id']; ?>"
                      data-material="<?php echo $value['ItemTransferencia']['materiais_id']; ?>"
                      data-quantidade="<?php echo $value['ItemTransferencia']['quantidade_transferida']; ?>"
                      data-data_hora_registro="<?php echo date('Y-m-d'); ?>"
                      data-pedidos_id="<?php echo $value['ItemTransferencia']['pedidos_id']; ?>"
                      data-patrimonio="<?php echo $value['Material']['quantidade']; ?>"
                ...
                <?php if(sizeof($entradas)){ ?>
                  <?php foreach ($entradas as $value) { ?>
                    <option
                      ...
                      data-transferencia_id="<?php echo $value['Entrada']['transferencia_id']; ?>"
  ...
  $(function () {
    $('#add_item').click(function() {
      ...
      $.each(associados, function( index, value ) {
        $.post('/patrimonio/transferencias/entrada_filial/', {
          ...
          'data[Entrada][transferencia_id]': $(value).data('transferencia_id'),
    ...
    $('#remove_item').click(function() {
      ...
      $.each(associados, function( index, value ) {
        $.post('/patrimonio/transferencias/delete_entrada/', {
          'data[Material][id]': $(value).data('materiais_id'),
          'data[Material][quantidade]': $(value).data('quantidade_entrada'),
          'data[Material][transferencia_id]': $(value).data('transferencia_id'),
