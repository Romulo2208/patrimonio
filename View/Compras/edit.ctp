<div class="widget-box">
  <div class="widget-header">
    <h4>EDITAR REQUISI&Ccedil;&Atilde;O DE COMPRAS</h4>
  ...
  <div class="widget-body" style="overflow-y: auto; max-height: 400px; min-width: 650px;">
    <div class="widget-main">
      <?php
      echo $this->Form->create('Compra', array('class' => 'form-horizontal'));
      ...
      <div class="row-fluid">
          <div class="input-prepend">
            ...
            <select name="data[Pedido][setores_id]" class="input-xlarge select" onchange="getDescricao(this);">
              ...
              foreach ($setores as $key => $value) {
                $selected = $key == $this->request->data['Compra']['setores_id'] ? 'selected' : null;
          ...
          <div class="input-prepend">
              ...
              <?php echo $this->Form->input('situacao', array('label' => false, 'div' => false, 'type' => 'select', 'options' => array('1'=>'Pendente','2'=>'Em Andamento', '3'=>'Aprovado', '4'=>'Bloqueado', '5'=>'Em Aberto'), 'required' => true, 'class' => 'input-medium')); ?>
          </div>
      </div>
      ...
      <span class="materiais" >
        <?php foreach ($itens as $id => $item) { ?>
          <div class="row-fluid" style="margin-top: 10px;">
              ...
              <span class="input-icon">
                <select name="material[id][]" class="input-xxlarge select" onchange="getDescricao(this);">
                  ...
                  foreach ($materiais as $key => $value) {
                    $selected = $value['Material']['id'] == $item['ItemCompra']['materiais_id'] ? 'selected' : null;
            ...
            <div class="input-prepend">
              ...
              <span class="input-icon input-icon-right">
                <input type="number" name="material[compra][]" class="input-mini" min="0" style="width: 40px;" value="<?php echo $item['ItemCompra']['quantidade_pedido']; ?>" />
              </span>
            </div>
            ...
            <div class="input-prepend">
              ...
              <span class="input-icon input-icon-right">
                ...
                <input type="hidden" name="material[item][]" value="<?php echo $item['ItemCompra']['id']; ?>" />
                <input type="hidden" name="material[pedidos_id][]" value="<?php echo $item['ItemCompra']['pedidos_id']; ?>" />
                <input type="hidden" name="material[comprou][]" value="<?php echo $item['ItemCompra']['comprou']; ?>" />
                <input type="hidden" name="material[equipamentos_id][]" value="<?php echo $item['ItemCompra']['equipamentos_id']; ?>" />
              </span>
            </div>
          </div>
        <?php } ?>
        ...
        <div class="row-fluid" style="margin-top: 10px;">
          <div class="input-prepend">
            ...
            <span class="input-icon">
              <select name="material[id][]" class="input-xxlarge select" onchange="getDescricao(this);">
                ...
                <?php
                foreach ($materiais as $key => $value) {
                  echo "<option value='{$key}' {$selected}>{$value['Material']['nome']}</option>";
                }
                ?>
              </select>
              <input type="hidden" name="material[descricao][]" class="input-medium descricao" />
          ...
          <div class="input-prepend">
            ...
            <span class="input-icon input-icon-right">
              <input type="number" name="material[compra][]" class="input-mini quantidade" min="0" style="width: 40px;" />
            </span>
          </div>
