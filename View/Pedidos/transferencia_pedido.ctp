              ...
              <select name="select1" id="select1" multiple="multiple" style="width: 100%; height: 250px;">
                <?php if(sizeof($itens)){ ?>
                  <?php foreach ($itens as $value) { ?>
                    <?php if($value['ItemPedido']['quantidade_fornecido'] !='' && $value['ItemPedido']['material_transferido']  != 1){ ?>
                    <option
                      ...
                      data-setor_id="<?php echo $value['Pedido']['setores_id']; ?>"
                      data-pedidos_id="<?php echo $value['ItemPedido']['pedidos_id']; ?>"
                      data-equipamentos_id="<?php echo $value['ItemPedido']['equipamentos_id']; ?>"
                      ><?php echo $value['ItemPedido']['quantidade_fornecido']. ' - ' .$value['Material']['nome'] ; ?></option>
                  <?php }}} ?>
              </select>
  ...
  $(function () {
    $('#add_item').click(function() {
      ...
      $.each(associados, function( index, value ) {
        $.post('/patrimonio/pedidos/transferencia_filial/', {
          ...
          'data[ItemTransferencia][quantidade_transferida]': $(value).data('quantidade_fornecido'),
    ...
    $('#remove_item').click(function() {
      ...
      $.each(associados, function( index, value ) {
        $.post('/patrimonio/pedidos/delete_transferencia/', {
          'data[ItemTransferencia][materiais_id]': $(value).data('material'),
