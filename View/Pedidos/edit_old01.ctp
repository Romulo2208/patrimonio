      ...
      <span class="materiais" >
        <?php foreach ($itens as $id => $item) { ?>
          <div class="row-fluid" style="margin-top: 10px;">
            ...
            <?php if(!in_array($this->Session->read('Perfil.id'), array('1','2','3','4','5','6','7'))) { ?>
            <div class="input-prepend">
              <label>Estoque</label>
                <select name="" class="input-small select" disabled="disabled" onchange="getDescricao(this);">
                  ...
                  foreach ($materiais as $key => $value) {
                    $selected = $value['Material']['id'] == $item['ItemPedido']['materiais_id'] ? 'selected' : null;
                    echo "<option value='{$value['Material']['id']}' {$selected}>{$value['Material']['quantidade']}</option>";
                  }
                  ?>
                </select>
            </div>
            ...
            <div class="input-prepend">
              ...
              <span class="input-icon input-icon-right">
                <input type="text" name="material[aplicacao][]" class="input-medium" min="0" value="<?php echo $item['ItemPedido']['aplicacao']; ?>" />
              </span>
            </div>
