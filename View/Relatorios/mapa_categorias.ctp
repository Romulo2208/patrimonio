...
<input type="text" id="max" placeholder="Data Final" name="material[final][]" data-date-format = "dd/mm/yyyy" style="width: 200px; float: left;"/>
  ...
  <div class="input-prepend">
    ...
    <select name="material[categoria][]" class="input-xlarge select" onchange="getDescricao(this);">
...
<table id="dyntable" class="table table-striped table-bordered table-hover">
  <thead>
    <tr bgcolor="#A9D08E">
      ...
      <th>CALTA</th>
      <th>AMERICAL</th>
      <th>FERRAGENS FL01</th>
      <th>SALDO GERAL</th>
  </thead>
  <tbody>

    <?php if(isset($this->request->data['material']['categoria'][0])) { ?>
    <?php foreach ($materiais as $material): ?>
      <tr>
        ...
        <td><?php echo h($material[0]['Ferragensfl01']); ?>&nbsp;</td>
        <td><?php echo h($material['a']['quantidade_central'] + $material[0]['Filial1']+$material[0]['Filial2']+$material[0]['Filial4']+$material[0]['Filial9']+$material[0]['Emfol']+$material[0]['Calta']+$material[0]['Americal']+$material[0]['Ferragensfl01']); ?>&nbsp;</td>
      </tr>
    <?php endforeach; ?>
  <?php } ?>
  </tbody>
